...
import (
	...
	listergenargs "github.com/caicloud/clientset/cmd/listerfactory-gen/args"
)
...
func Packages(context *generator.Context, arguments *args.GeneratorArgs) generator.Packages {
	...
	// Note: parse the file to get all the right interface with methods in listers
	// extra Context
	b := parser.New()
	b.AddDirRecursive("k8s.io/client-go/listers")
	extraCtx, err := generator.NewContext(b, NameSystems(), DefaultNameSystem())
	if err != nil {
		panic(err)
	}
	...
	for _, inputDir := range arguments.InputDirs {
		...
		if internal {
			packageList = append(packageList, versionPackage(internalVersionPackagePath, groupPackageName, gv, groupGoNames[groupPackageName], boilerplate, typesToGenerate, customArgs.InternalClientSetPackage, customArgs.ListersPackage, extraCtx))
		} else {
			packageList = append(packageList, versionPackage(externalVersionPackagePath, groupPackageName, gv, groupGoNames[groupPackageName], boilerplate, typesToGenerate, customArgs.VersionedClientSetPackage, customArgs.ListersPackage, extraCtx))
		}
	}

	if len(externalGroupVersions) != 0 {
		packageList = append(packageList, factoryInterfacePackage(externalVersionPackagePath, boilerplate, customArgs.InternalClientSetPackage))
...
func versionPackage(basePackage string, groupPkgName string, gv clientgentypes.GroupVersion, groupGoName string, boilerplate []byte, typesToGenerate []*types.Type, clientSetPackage, listersPackage string, extraCtx *generator.Context) generator.Package {
