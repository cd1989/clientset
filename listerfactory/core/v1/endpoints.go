...
func (s *endpointsLister) List(selector labels.Selector) (ret []*core_v1.Endpoints, err error) {
	...
	list, err := s.client.CoreV1().Endpoints(meta_v1.NamespaceAll).List(listopt)
	if err != nil {
		return nil, err
	}
...
func (s endpointsNamespaceLister) List(selector labels.Selector) (ret []*core_v1.Endpoints, err error) {
	listopt := meta_v1.ListOptions{
		LabelSelector: selector.String(),
	}
	if s.tweakListOptions != nil {
...
func (s endpointsNamespaceLister) Get(name string) (*core_v1.Endpoints, error) {
	return s.client.CoreV1().Endpoints(s.namespace).Get(name, meta_v1.GetOptions{})
}
