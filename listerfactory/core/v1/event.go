...
func (s *eventLister) List(selector labels.Selector) (ret []*core_v1.Event, err error) {
	...
	list, err := s.client.CoreV1().Events(meta_v1.NamespaceAll).List(listopt)
	if err != nil {
		return nil, err
	}
...
func (s eventNamespaceLister) List(selector labels.Selector) (ret []*core_v1.Event, err error) {
	listopt := meta_v1.ListOptions{
		LabelSelector: selector.String(),
	}
	if s.tweakListOptions != nil {
...
func (s eventNamespaceLister) Get(name string) (*core_v1.Event, error) {
	return s.client.CoreV1().Events(s.namespace).Get(name, meta_v1.GetOptions{})
}
